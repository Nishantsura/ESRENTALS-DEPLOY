...
import { searchAPI, carAPI } from "@/services/api";
  ...
  useEffect(() => {
    const searchCars = async () => {
      ...
      try {
        ...
        // Fetch additional data from Supabase API for each result
        const enhancedResults = await Promise.all(
          searchResults.map(async (car) => {
            try {
              ...
              if (carData) {
                // Merge Supabase data with Algolia data, prioritizing Supabase
                return {
                  ...car,
                  ...carData,
                  id: carId, // Ensure we keep the ID
                  objectID: car.objectID, // Keep Algolia objectID
                  dailyPrice: carData.dailyPrice ?? car.dailyPrice, // Fallback to Algolia price
                  images: carData.images ?? car.images // Fallback to Algolia images
                } as Car;
              }
              return car;
            } catch (error) {
              console.error(`Error fetching Supabase data for car ${car.objectID || car.id}:`, error);
              return car;
            }
          })
        );
