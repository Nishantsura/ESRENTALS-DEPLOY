...
interface TiltedCardProps {
  ...
  className?: string;
  tooltipClassName?: string; // Added for tooltip theming
}
...
export const TiltedCard: FC<TiltedCardProps> = ({
  ...
  return (
    <figure
      ref={ref}
      className={`relative [perspective:800px] flex flex-col items-center justify-center overflow-hidden ${className}`}
      style={{
      ...
      onClick={() => {
        // For mobile devices, show overlay content on tap
        if (isMobile && displayOverlayContent) {
          scale.set(scaleOnHover);
    ...
    >
      ...
      >
        <motion.img
          src={imageSrc}
          alt={altText}
          className="absolute top-0 left-0 w-full h-full object-cover rounded-[12px] will-change-transform [transform:translateZ(0)]"
          loading="eager"
        />
